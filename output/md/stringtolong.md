...
The `StringToLong` function will extract a number from a string
variable, and store it in a Long variable. One potential use is reading
numbers that are sent in ASCII format over a serial connection.  
  
The `StringToLong` function will not extract a StringToLong from a
hexadecimal string.  
