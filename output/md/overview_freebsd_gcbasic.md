...
program installed, you can either compile it from the ports collection
or use the `pkg` command to install the binary and any required
dependencies.
...
2.  For Atmel AVR microcontroller programming, you will need the
    ...
    installed from the FreeBSD ports directory, or the precompiled
    binary and any missing dependencies can be installed using
    `pkg install avrdude`.

</div>

Alternatively, if you use virtual machine software such as <span
class="emphasis">*VirtualBox*</span> to run Windows programs, you may be
able to use Windows GUI programming software.
