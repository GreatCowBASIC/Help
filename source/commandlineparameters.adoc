...
(((Introduction,Command Line Parameters)))
...
----
    ...
    GCBASIC [/O:output.asm] [/A:assembler] [/P:programmer] [/K:{C|A}] [/H:[Y/1 | N/0]] [/V] [/L] [/WX] [/M:[Y/1 | N/0]]  [/S:Use.ini] [/F[O]] [/NP] _filename_ /DO

    GCBASIC [/O:output.asm] [/A:assembler] [/P:programmer] [/K:{C|A}] [/H:[Y/1 | N/0]] [/V] [/L] [/WX] [/M:[Y/1 | N/0]]  [/S:Use.ini] [/F[O]] [/NP] _filename_ /AO
...
|`/F[:[0\|F][1\|T]`
|Used to bypass compilation when not needed - the compiler will verify that the config settings in the already-compiled file match those required for the programmer. If not, a recompilation will be forced.
 Skip compilation if the hex file is up to date and has the correct config.
/F:x (F or 0) to force a fresh compile regardless of what the ini file specifies.
|
...
|`/O:__filename__`
|Sets the name of the generated assembly file to `_filename_`.
...
|`/V:[0\|F][1\|T]`
|Verbose mode -- the compiler gives more detailed information about its activities. `/Vx` will override any configuration in the user ini file.
|-
...
This example will use MPASM to assemble the program. It will run the program specified on the assembler = line, and give it these parameters:
...
* <<_using_gcbasic,Using GCBASIC>> -- compiling and flashing from the IDE instead of the command line
* <<_about_the_preferences_editor,About the Preferences Editor>> -- the GUI tool for editing use.ini
