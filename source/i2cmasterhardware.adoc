...
(((Example Programs,I2C Master Hardware)))
...
----
*Key line:* `eeprom_wr_array(EEpromDevice, 64, location, outarray, 64)` -- writes all 64 bytes of `outarray` to the I2C EEPROM in one call, using the library's page-write function; the matching `eeprom_rd_array` call just below reads them back so the two arrays can be compared byte-for-byte.

**See Also:**

* <<_i2c_slave_hardware,I2C Slave Hardware>> -- the corresponding slave-side example
* <<_hi2c_overview,HI2C Overview>> -- the hardware I2C command reference
