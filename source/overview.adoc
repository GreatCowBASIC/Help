...
----
  ...
  End
----
*Key line:* `I2CSend ( deviceID )` -- addresses each possible device ID in turn; `I2CSendState` afterward tells the program whether that device acknowledged, which is how this sketch discovers what is present on the bus.
Supported in <I2C.H>
