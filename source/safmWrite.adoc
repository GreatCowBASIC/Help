...
{empty} +
{empty} +
`location` represents the location or relative address to write. The location ranges from location 0 to `SAF_BYTES - 1`, or for all practical purposes 0-255, since all PIC microcontrollers with SAFM support 256 bytes of SAF memory.&#160;&#160;&#160;
`SAF_BYTES` is a GCBASIC constant that represents the number of bytes of SAF memory.    
{empty} +
{empty} +
`data` is the data to be written to the SAFM location.&#160;&#160;&#160;
