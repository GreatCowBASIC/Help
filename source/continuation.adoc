...
A single _ (underscore) character at the end of a line of code tells the compiler that the line continues on the next line. This allows a single statement (line of code) to be spread across multiple lines in the input file, which can provide nicer formatting.
...
* <<_subroutines,Subroutines>> -- declaring a Sub, as split across lines in Example 2
* <<__define,#DEFINE>> -- creating a constant, as split across lines in Example 3
