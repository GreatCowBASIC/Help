...
If `numeric constant` is specified then the compiler will complete a simple math calculation to determine the frequency.&#160;&#160;The only supported math instruction is divide.  
...
When using an AVR:
 ...
 . Only AVRDX chips support setting the internal frequency using the `frequency` statement.
...
When using a LGT:
 ...
 . Only LGT support setting the internal frequency using the `frequency` statement.
...
Each microcontroller has a microcontroller data file.  This file is located in \GCBasic\chipdata\ folder when installed.
