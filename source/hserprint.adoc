...
(((Serial Communications,HSerPrint)))
...
* <<_hserprintbytecrlf,HserPrintByteCRLF>> -- sends a byte then a CRLF terminator
* <<_hserprintstringcrlf,HserPrintStringCRLF>> -- sends a string then a CRLF terminator
* <<_hserprintcrlf,HserPrintCRLF>> -- sends just a CRLF terminator
* <<_hserreceive,HSerReceive>> -- the receiving counterpart used earlier in this example
* <<_hsersend,HSerSend>> -- sending a single raw byte, as used to send the trailing CR/LF above
* <<_hsergetstring,HSerGetString>> -- reading a whole string from the serial connection in one call
