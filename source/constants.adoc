...
string, and replace it with another string. The `#Define` directive creates constants.
...
Creating a constant is a matter of using the `#define` directive. Here are
...
----
`LINEPARAMETER` is a simple constant - GCBASIC will find `LINEPARAMETER` in the program, and
replace it with the number 34. This could be used in a line of the following
...
{empty} +
{empty} +
The `#define` command creates constants, and scripts also create constants. See <<__script,#script>> for more on this.
{empty} +
{empty} +
...
* <<__define,#DEFINE>> -- the directive that creates constants
* <<__script,#script>> -- constants created at compile time by scripts
