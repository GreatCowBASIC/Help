...
* <<_repeat,Repeat>> -- fixed-count loop that does not need a counter variable
* <<_do,Do>> -- condition-driven loop for when the number of iterations is not known in advance
* <<_dim,Dim>> -- declaring the loop counter/variable types used above
* <<_pulseout,PulseOut>> -- generating a timed pulse, as used in the example above
* <<_if,If>> -- conditionally exiting the loop with `Exit For`
