...
(((Variables,The Positives and Negatives of Variable Types)))
...
Computers represent numbers in binary form. The size of a number is determined by the width of the memory or register storing it, and by the capabilities of the Arithmetic Logic Unit (ALU) that processes it.
...
*Single* varaible store 64-bit signed values with seven decimal digits of precision.  numeric range of approximately 10^−38 to 10^38.  
...
[start=1]
. The usable magnitude is reduced because one bit is reserved for the sign.
. A negative zero is possible unless avoided.
. Most ALUs do not implement subtraction directly; subtraction is performed using addition of a two’s complement value.
...
* <<_advanced_variabletypes,Advanced VariableTypes>> — the `Single` floating‑point type  
* <<_data_types,Data Types>> — the full list of GCBASIC variable types  
* <<_variables_and_data_types,Variables and Data Types>> — byte‑level access to multi‑byte variables
