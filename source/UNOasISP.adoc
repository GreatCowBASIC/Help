...
(((GCBASIC Compiler Insights,UNO as ISP programmer)))



So, you have bought some ATtiny88 breakout boards online.
...
NOTE: This refers to an ATtiny88, but you can use this method for many AVRs used in conjunction with AVRdude.


*The Process*

1. Obtain an Arduino UNO or Mega.
Upload this https://sourceforge.net/p/gcbasic/discussion/chipfileforum/thread/088449090a/ed10/attachment/arduino_As_ISP_adafruit.hex[hex file] to convert the UNO into an ISP programmer, or follow steps 2-5 below.
...
In the Arduino IDE, go to Tools -> Set board and select "Arduino UNO".
Select the correct com port for the Arduino UNO, as shown in Device Manager.

4. Go to File -> Examples -> ArduinoISP to select the sketch that will convert the UNO to an ISP programmer. A better-working version is available at Adafruit. Simply copy all the text from this link into a new sketch, https://raw.githubusercontent.com/adafruit/ArduinoISP/master/ArduinoISP.ino[ArduinoISP.ino] (or download the ino file and open it in the Arduino IDE), and go to step 5.
...
_Enter the sample code below into the GCB IDE_
...
* <<_setup_an_avrisp_mkii_or_usbtinyisp_for_attiny10_chip_under_windows,Setup an AVRISP MKII or USBtinyISP for ATTINY10 chip under Windows>> -- an alternative approach using a dedicated AVRISP MKII or USBtinyISP programmer
* <<_set,Set>> -- setting a pin on or off, as used above
