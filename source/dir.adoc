...
* <<_set,Set>> -- setting a pin's output state after configuring its direction
* <<_pwmout,PWMOut>> -- an example that sets a pin's direction before driving it
* <<_readad,ReadAD>> -- reading an analog pin, which does not need an explicit Dir
