...
(((Example Programs,Generate Accurate Pulses)))


*Explanation:*

The `PulseOut` command is a reliable method for generating pulses if accuracy is not critical;
the `PulseOut` command uses a calculation of the clock speed for the timing.
...
`Timer1` is loaded with a preset value based upon the variable passed to the subroutine.
...
exit a polling loop and set the pulse pin off. Then, `Timer1` is stopped, the `TMR1IF` flag is cleared, and the subroutine exits.
