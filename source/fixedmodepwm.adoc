...
(((Pulse width modulation,HPWM Fixed Mode)))
...
----
*Key line:* `HPWMOn ( 7, PWMModule )` -- starts PWM module 7 using the frequency, duty, and clock-source constants (`PWM_Timer2_Freq`, `PWM_7_Duty`, `PMW_7_Clock_Source`) defined above; `PWMModule` tells `HPWMOn` this is a PWM-module channel rather than a CCP channel.
...
* <<_pwmon,PWMOn>> -- full reference for the legacy channel-1 form
* <<_pwmoff,PWMOff>> -- stopping the PWM output
* <<_hpwm_fixed_mode_for_avr,HPWM Fixed Mode for AVR>> -- the Atmel AVR equivalent
* <<_hpwm_ccp,HPWM CCP>> -- the dynamic-mode alternative for changing frequency and duty cycle at runtime
