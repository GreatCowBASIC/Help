...
----
    ...
    next i
----
*Key line:* `sqrt(i)` -- computes the integer square root of the loop counter `i`, scaled as a fixed-point value that the surrounding `select case` block then reformats with a decimal point.
*Supported in <MATHS.H>*

**See Also:**

* <<_power,Power>> -- the inverse-square relationship to Sqrt
