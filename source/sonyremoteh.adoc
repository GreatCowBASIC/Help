...
----
*Key line:* `IR_rem_key = IR_rem_key + 64` -- a measured pulse width over 10 units is decoded as a binary 1 and added into the value being built; because the loop divides by 2 each pass first, this progressively shifts each new bit into the correct position, reconstructing the 7-bit key code from the raw pulse widths captured earlier.

**See Also:**

* <<_infrared_remote,InfraRed Remote>> -- the worked example program using this header
