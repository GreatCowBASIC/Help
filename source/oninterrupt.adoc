...
* <<_on_interrupt_the_default_handler,On Interrupt: The default handler>> -- what happens to events not claimed by an `On Interrupt` line
* <<_intoff,IntOff>> -- globally disabling interrupts
* <<_inton,IntOn>> -- globally re-enabling interrupts
* <<_inittimer0,InitTimer0>> -- an example using Timer 0 and On Interrupt to generate a PWM signal to control a motor
* <<_inittimer1,InitTimer1>> -- configuring Timer1, as used in Example 1 above
