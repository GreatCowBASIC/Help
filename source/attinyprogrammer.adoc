...
(((GCBASIC with the AVRISP or MKII Programmer,Setup an AVRISP MKII or USBtinyISP for ATTINY10 chip under Windows)))
...
When using AVRDUDE, which is used as part of the GCBASIC toolchain, you need to ensure these programmers are operating with the correct Windows device driver.
The Windows driver must be `libusbK`, not the `libusb` or `libusb-win32` Windows driver.
...
* Download and install Zadig Driver Utility software https://zadig.akeo.ie/[Zadig]
...
* Download and install the latest version of "Atmel Flip" software (it can be downloaded from Atmel's website, or from the GCBASIC file store https://sourceforge.net/projects/gcbasic/files/Support%20Files/ATMELCompilers/AVRISPMk2/[here])
...
* Download the firmware and unpack it from the https://www.olimex.com/Products/AVR/Programmers/AVR-ISP-MK2/resources/AVR456-studio-AVRISP-MKII.zip[Olimex website] or the GCBASIC file store https://sourceforge.net/projects/gcbasic/files/Support%20Files/ATMELCompilers/AVRISPMk2/[here]
...
For more information about AVR-ISP-MK2, see this guide https://gcbasic.sourceforge.net/library/AVR-ISP-MK2-OpenSourceHardwareBoard.pdf[here]
...
* <<_uno_as_isp_programmer,UNO as ISP programmer>> -- an alternative approach using an Arduino UNO instead of a dedicated programmer
