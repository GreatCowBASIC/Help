...
(((Variables Operations,Setting Variables)))
...
* <<_variable_types,Variable Types>> -- background on GCBASIC's variable types and casting
* <<_using_variables,Using Variables>> -- accessing individual bytes of Word/Long variables with the ` _H `/` _U `/` _E ` suffixes
* <<_dim,Dim>> -- declaring the variables being set here
