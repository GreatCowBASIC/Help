...
(((Liquid Crystal Display,lcd_speed)))
...
With most displays this equates to a speed of about 30,000 characters per second.  For comparision about 10 times faster than I2C using a PC8574 Expander (See <<_lcd_io_10,LCD_IO 10>> or <<_lcd_io_12,LCD_IO 12>>)
...
* <<_lcd_io_4,LCD_IO 4>> -- 4-bit connection mode, required for OPTIMAL speed
* <<_lcd_io_8,LCD_IO 8>> -- 8-bit connection mode, required for OPTIMAL speed
* <<_lcd_io_10,LCD_IO 10>> -- I2C connection mode, compared for speed above
* <<_get,Get>> -- reading a byte from the LCD, requires busy-flag checking to be available
