...
* <<_starttimer,StartTimer>> / <<_stoptimer,StopTimer>> -- starting and stopping the timer configured here
* <<_settimer,Settimer>> -- preloading the timer's count value, as used above
* <<_on_interrupt,On Interrupt>> -- attaching an interrupt handler to the timer overflow, as used above
* <<_cleartimer,ClearTimer>> -- resetting the timer's count to zero
* <<_pulseout,PulseOut>> -- generating the LED pulse in Example 2
