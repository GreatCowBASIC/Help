...
(((Compiler Options,#Option NoLatch)))
...
GCBASIC resolves this issue by using the LATx register when writing to ports, rather than using PORTx registers. Writing to a LATx register is equivalent to writing to a PORTx register, but readings from LATx registers return
...
* <<__option_volatile,#Option Volatile>> -- resolving a related class of read-modify-write glitch on other pins
* <<_dir,Dir>> -- setting a pin's direction
