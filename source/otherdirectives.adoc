...
(((Compiler Directives,Other directives)))
...
{empty} + If none of the bits exist, no constant is created.
...
{empty} + If none of the variables exist, no constant is created.
...
* <<__ifdef,#ifdef>> -- the directive these Var/NoVar/Bit/NoBit/AllOf/OneOf functions are used inside
* <<__chip,#chip>> -- selecting the target chip these constants describe
