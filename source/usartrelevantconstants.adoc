...
(((Serial Communications,RS232 Hardware Overview)))
...
|`USART4_BAUD_RATE` |Baud rate (in bps) for the routines to operate at. |No default, user must enter a baud. Does not have to be a standard baud.
|`USART4_BLOCKING`
|If defined, this constant will cause the USART routines to wait until data
can be sent or received.
|No parameter needed. Use “#defining” it implement the action.
...
* <<_rs232_software_overview,RS232 Software Overview>> -- related command in the same category
* <<_initser,InitSer>> -- related command in the same category
