...
The method uses the `RaiseCompilerError` method to pass error message to the assembler (not the compilation process).
...
This is hard coded into the compiler (at build 1131).
{empty} +
{empty} +
