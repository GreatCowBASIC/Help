...
(((Interrupts,On Interrupt: The default handler)))
...
----
    ...
    End Sub
----
*Key line:* `TMR1IE = 1` -- enables the Timer1 overflow interrupt directly, with no matching `On Interrupt` line, so every occurrence of the event falls through to the default `Interrupt` subroutine.
*Example 2*
...
* <<_on_interrupt,On Interrupt>> -- registering a handler for a specific interrupt event
* <<_intoff,IntOff>> -- globally disabling interrupts
* <<_inton,IntOn>> -- globally re-enabling interrupts
* <<_inittimer1,InitTimer1>> -- configuring Timer1, as used in both examples above
