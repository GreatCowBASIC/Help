...
----
*Key line:* `outString1 = leftpad(inString, 9, "{asterisk}")` -- pads `inString` ("12345") out to 9 characters by adding `{asterisk}` characters on the left, giving `"{asterisk}{asterisk}{asterisk}{asterisk}12345"`; the second call omits the pad character and defaults to padding with blank spaces instead.

**See Also:**

* <<_left,Left>> -- related command in the same category
