...
----
    ...
    Loop
----
*Key line:* `Rotate PORTB Right Simple` -- shifts every bit of PORTB one place right, with the `Simple` option wrapping the bit that falls off the LSB back around to the MSB, so the two lit LEDs appear to chase around the port continuously.
...
* <<_set,Set>> -- clearing the STATUS.C / carry flag before rotating, as recommended above
