...
(((MCU EEPROM ( DFM ),Dataset for EEPROM)))
...
. TABLE data has precedence from address 0x00 until the end of TABLE all data,
...
. 18F devices must use even address for EEPROM location, and, 18F will pad (with 0x00) datasets to even number length.&#160;&#160;This is MPASM constraint and therefore the compiler and assembler will issue specific error messages for odd EEPROM locations.   
{empty} +
{empty} +
