...
----
    ...
    3. Process the user source programs using PreProcessor method. This includes
        ...
        iv.    Find compiler directives, except SCRIPT, ENDSCRIPT, IFDEF and ENDIF
        v.     ReadChipData
        vi.    CheckClockSpeed
        viii.  ReadOptions
        ix.    PreparePageData
        ...
        xiv.   RemIfDefs.  Remove any #IFDEFs that do not apply to the program. Prepare programmer, need to know chip model and need to do this before checking config
        xv.    Replace Constants
        xvi.   Replace table value.  Replace constants and calculations in tables with actual values
