...
----
2. Edit `c:\docbook-xsl-ns\htmlhelp\xsl\htmlhelp.xsl` and insert after the `xsl:include` statement
the same 3 lines of code as follows:
+
----
...
ant -version
----
The `ant` version should show up. You can ignere the message "Unable to locate tools.jar".
----
C:\Users\GCB>ant -version
Unable to locate tools.jar. Expected to find it in C:\Program Files (x86)\Java\jre1.8.0_45\lib\tools.jar
...
GCBASIC documentation is splitted in small `sectioname.adoc` text files which are then merged in a `gcbasic.adoc`
one.

The syntax used in GCB `gcbasic.adoc` file is as follows:
...
The `source` subdir contains all the `*.adoc` files including `gcbasic.adoc`. It also
contains:
...
call java com.icl.saxon.StyleSheet ..\..\source\%1.xml %GCBstyleHTML% ^
 chunk.section.depth=5 htmlhelp.remember.window.position=1 htmlhelp.hhp="%1.hhp" htmlhelp.hhc="%1.hhc" ^
 htmlhelp.chm="%1.chm"  htmlhelp.title="GCBASIC Help" generate.toc=" " use.id.as.filename=1
if ERRORLEVEL 1 goto :ERROR
call hhc %1.hhp
del *.htm*
del *.hh*
