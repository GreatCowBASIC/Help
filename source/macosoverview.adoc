...
(((GCBASIC for Apple macOS,Overview - Apple macOS GCBASIC)))
...
[start=1]
 . Download the GCBASIC - macOS Installer disk image (.dmg) file from https://sourceforge.net/projects/gcbasic/files/GCBasic-macOS-Installer.dmg/download[sourceforge.net]

 . Double click the .dmg file to mount it on your Desktop, and a window will open which contains the Installer.

 . Double click the README_FIRST.txt file and read it for any important information you may need before proceeding.
...
[start=1]
 . Download the FreeBASIC 1.06 macOS binary compilation from: https://tmc.castleparadox.com/temp/fbc-1.06-darwin-wip20160505.tar.bz2[castleparadox.com]
...
. For Microchip PIC microcontroller programming, you might find what you need at: https://www.microchip.com/DevelopmentTools/ProductDetails.aspx?PartNO=pg164120[Microchip Development Tools] and the macOS version of the `pk2cmd` v1.2 command line programming software.

. For Atmel AVR microcontroller programming, you will need the `avrdude` programming software. Check here: https://www.nongnu.org/avrdude/[avrdude website]
...
* <<_overview_freebsd_gcbasic,Overview - FreeBSD GCBASIC>> -- the equivalent instructions for FreeBSD
