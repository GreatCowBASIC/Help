...
The condition `bit_variable = other_type_of_variable` generates a warning. If the byte_variable is set to TRUE and then compared to the bit, it will always be FALSE, because the high bit will be treated as a 1. The warning generated is: "Comparison will fail if %nonbit% is any value other than 0 or 1".
...
* <<_if,If>> -- using conditions to branch
* <<_do,Do>> -- using conditions to loop, as in the examples above
* <<_hserreceive,HSerReceive>> -- the function used in the worked example
